...
object JdbcProvisioner {
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      println("Creando la tabla metadata (timestamp TIMESTAMP, id TEXT, antenna_id TEXT, bytes BIGINT, app TEXT)")
      statement.execute("CREATE TABLE IF NOT EXISTS metadata (timestamp TIMESTAMP, id TEXT, antenna_id TEXT, bytes BIGINT, app TEXT)")
